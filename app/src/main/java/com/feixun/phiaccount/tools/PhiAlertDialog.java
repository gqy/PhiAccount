...
public class PhiAlertDialog extends AlertDialog implements DialogInterface {
    private PhiAlertController mAlert;
    private ViewGroup mViewGroup;
    private Activity activity;
    private LinearLayout mViewGroupView;
    private boolean mIswithoutExitAnimation = false;

    protected PhiAlertDialog(Context context) {
        super(context, R.style.alertdialog_theme_alert);
        setCanceledOnTouchOutside(true);
        mAlert = new PhiAlertController(context, this, getWindow());
        ViewGroup rootView = (ViewGroup) ((ViewGroup) ((Activity) context)
                .getWindow().getDecorView().findViewById(android.R.id.content))
                .getChildAt(0);
        this.mViewGroup=rootView;
        this.activity = (Activity) context;
    }

    protected PhiAlertDialog(Context context, int theme) {
        super(context, theme);
        setCanceledOnTouchOutside(true);
        mAlert = new PhiAlertController(context, this, getWindow());
    }

    protected PhiAlertDialog(Context context, boolean cancelable,
    ...
    protected PhiAlertDialog(Context context, Activity activity, ViewGroup view){
        super(context, R.style.alertdialog_theme_alert);
        setCanceledOnTouchOutside(true);
        mAlert = new PhiAlertController(context, this, getWindow());
    ...
    protected PhiAlertDialog(Context context, Activity activity, ViewGroup view, boolean isWithoutExitAnimation){
        super(context, R.style.alertdialog_theme_alert);
        setCanceledOnTouchOutside(true);
        mAlert = new PhiAlertController(context, this, getWindow());
        this.mViewGroup=view;
        this.activity=activity;
        this.mIswithoutExitAnimation = isWithoutExitAnimation;
    }
    private void init(Context context){
        Window window = this.getWindow();
        WindowManager.LayoutParams lp = window.getAttributes();
        if(mIswithoutExitAnimation){
            lp.windowAnimations = R.style.fxlib_phialertdialog_insert_style_without_exit_animation;
        }else{
            lp.windowAnimations = R.style.fxlib_phialertdialog_insert_style_animation;
        }
        lp.width= ViewGroup.LayoutParams.MATCH_PARENT;
        lp.height= ViewGroup.LayoutParams.WRAP_CONTENT;
        mViewGroupView=(LinearLayout) getWindow().findViewById(R.id.fxlib_phialertdialog_pcenterPanel);
    }
    @Override
    public void show() {
        // TODO Auto-generated method stub
        if (activity != null && mViewGroup != null) {
            super.show();
            mViewGroupView.setVisibility(View.VISIBLE);
        }
    ...
    /**
     * @see com.feixun.fxcloud.view.dialog.PhiAlertDialog.Builder#setCustomTitle(View)
     */
    ...
    public static class Builder extends AlertDialog.Builder {
        private final PhiAlertController.AlertParams P;
        private Activity mActivity;
        private ViewGroup mViewGroup;
        private PhiAlertDialog dialog;
        private boolean isWithoutExitAnimation = false;

        public Builder(Context context) {
            super(context);
            P = new PhiAlertController.AlertParams(context);
        }
        public Builder(Context context,Activity activity,ViewGroup view) {
            super(context);
            P = new PhiAlertController.AlertParams(context);
            this.mActivity = activity;
            this.mViewGroup = view;
        }
        public Builder(Context context,Activity activity,ViewGroup view,boolean withoutExitAnimation) {
            super(context);
            P = new PhiAlertController.AlertParams(context);
            this.mActivity = activity;
            this.mViewGroup = view;
            this.isWithoutExitAnimation = withoutExitAnimation;
        }
        ...
        public PhiAlertDialog create() {
            if(mActivity == null && mViewGroup == null ){
                dialog = new PhiAlertDialog(P.mContext);
            }else{
                if(isWithoutExitAnimation){
                    dialog = new PhiAlertDialog(P.mContext,mActivity,mViewGroup,isWithoutExitAnimation);
                }else{
                    dialog = new PhiAlertDialog(P.mContext,mActivity,mViewGroup);
                }
